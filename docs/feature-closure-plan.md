...
| **P1-3** | **音效 / BGM** | ✅ 已交付（音效） | ~~全仓 0 个 `AudioSource`~~ → `AudioConfig` / `SfxSynth` / `SfxRecipes` / `AudioClipFactory` / `AudioDirector` / `AmbienceLayer`（阶段 23）。**12 个音效 + 1 条环境衬底全部程序化合成，零二进制资源**；16 路 voice 池 + 四层闸门；53 条测试。内核零改动，只订阅其 `PlaySfx` 事件。**旋律 BGM 本期不做**（程序化生成的旋律不如没有），已留扩展位。**待用户本地验证** | 🔴 Unity（内核仅出事件 🟢） |
